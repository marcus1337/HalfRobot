...
using namespace std;
 ...
 void PhysicsPlayer::update(Model& model, Entity& entity) {
	...
	/* if (entity.getX() < 800)
		 cout << "Y: " << entity.getY2() << "  blocks: " << ((entity.getY2()) / 20)
		 << "  real: " << 20- (entity.getY2() -(((entity.getY2()) / 20)*20)) << endl;*/
	 y2 = (entity.getY() + entity.getHeight()/2) / 20;
	 if (xVel > 0) {
		 xPathBlocked = true;
		 int x5 = (entity.getX2() + xVel) / 20;
		 if (karta[x5 + y1 * bw] != 1 && karta[x5 + y3 * bw] != 1 && karta[x5 + y2 * bw] != 1) {
 ...
 bool PhysicsPlayer::isOnLadder() {
	 return onLadder;
 }


 bool PhysicsPlayer::isInAir() {
	 return inAir;
 }


 int PhysicsPlayer::checkYCollision(char* karta, int xLeft, int xMid, int xRight, int yUp, int yDown, int yVel) {
	 using namespace std;
 ...
 bool PhysicsPlayer::isInLadder(char* karta, int xLeft, int xMid, int xRight, int yDown) {
	 int y1 = (yDown - 1) / 20;
 ...
 void PhysicsPlayer::checkExitLadder(char* karta, int xLeft, int xRight, int yDown, int yDownP, int yUp, int yVel) {
	 if (!onLadder)
		 return;
	 using namespace std;
 ...
 void PhysicsPlayer::checkCanLadder(char* karta, int xLeft, int xRight, int yDown, int yUp) {
